 ...
 * const manifest = { name: 'my-package', bin: './cli.ts' }
 * const normalized = normalize(manifest)
 * // normalized.bin is now { 'my-package': './cli.ts' }
 *
 * @example
 * // normalizing a manifest with record bin
 * const manifest = { name: 'my-package', bin: { 'my-cli': './cli.ts' } }
 * const normalized = normalize(manifest)
 * // normalized.bin remains { 'my-cli': './cli.ts' }
 */
