...
Ts.test('NonPositive has correct relationships with other sign brands', () => {
  ...
  const _bad2: NonPositive = posVal
  ...
  const _npFromNeg: NonPositive = negVal
})
...
Ts.test('NonPositive is nominal (brand-based), not structural', () => {
  ...
  const good: NonPositive = nonPositive(-5)
})
...
Ts.test('NonPositive practical examples', () => {
  ...
  const checkFreezing = (temp: number): FreezingOrBelow | null => {
  ...
  const recordDebt = (amount: DebtOrZero): void => {
