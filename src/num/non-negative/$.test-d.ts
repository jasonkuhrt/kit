...
Ts.test('NonNegative has correct relationships with other sign brands', () => {
  ...
  const _nnFromPos: NonNegative = posVal
})
...
Ts.test('NonNegative enables specific numeric constraints', () => {
  ...
  const calculateSquareRoot = (value: NonNegative): number => {
...
Ts.test('NonNegative is nominal (brand-based), not structural', () => {
  ...
  const good: NonNegative = nonNegative(5)
})
...
Ts.test('NonNegative practical examples', () => {
  ...
  const getElement = <T>(arr: T[], index: ArrayIndex): T | undefined => {
    return arr[index]
  }
  ...
  const calculateArea = (width: Distance, height: Distance): number => {
