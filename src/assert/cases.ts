 ...
 * type _ = A.Case<Assert.exact<string, number>>  // May silently pass!
 *
 ...
 * ```ts
 * type MyTests = [
 *   A.Case<Equal<string, string>>,  // OK - evaluates to never (success)
 *   A.Case<Equal<string, number>>,  // Error - doesn't extend never (returns error)
 * ]
 * ```
 */
