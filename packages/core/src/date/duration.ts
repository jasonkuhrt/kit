 ...
 *
 * Provides structured duration formatting with automatic unit scaling,
 * useful for custom rendering of time intervals.
 */
...
export type Formatted = {
  ...
  value: number
  /**
   * The time unit (ms, s, m, h, d), or 'max' for very large durations.
   */
  unit: Unit | 'max'
}

/**
 * Format milliseconds into a structured duration with automatic unit scaling.
 *
 ...
 * - >= 10d: 'max'
 *
 * Unlike Effect's `Duration.format` which returns a ready-to-display string,
 * this returns structured data for custom rendering logic.
 *
