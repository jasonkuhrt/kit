...
- [**`Test`**](/api/ts/test) - Type-level assertion utilities for testing type correctness.
...
- [**`VariancePhantom`**](/api/ts/variance-phantom) - Phantom type helper that makes a type parameter covariant.
...
but adds additional styling to display the rendered type in a sentence.

Useful for type-level error messages where you want to clearly distinguish type names

from surrounding text. Wraps the printed type with backticks (

) like inline code in Markdown.
...
but uses single quotes instead of backticks.
...
only reach for this variant of

Show

if you think there is likelihood that types won't be instantiated.
...
- Intersection types that appear as
...
<SourceLink href="https://github.com/jasonkuhrt/kit/blob/main/./src/utils/ts/print.ts#L55" />
...
Simplify a type while preserving

| null

unions.

This solves a subtle problem with
...
can absorb or transform the

null

in unexpected ways

due to the intersection with
...
. This utility checks for null first,

then explicitly reconstructs the union to ensure
...
when simplifying types that may contain
...
for non-nullable types or when null handling doesn't matter
...
if type A does not extend type B,
...
absorbs all type operations including impossible intersections.
...
- everything extends unknown (except any, which is special).
...
### <span style="opacity: 0.6; font-weight: normal; font-size: 0.85em;">`[T]`</span> `IsEquivalent`

```typescript
type IsEquivalent<A, B> = [A] extends [B] ? [B] extends [A] ? true : false
  : false
```
...
type GetRelation<A, B> =
  ...
  [A] extends [B] ? [B] extends [A] ? 'equivalent' // Both extend each other - types are mutually assignable
    ...
    : A extends Lang.Primitive ? B extends Lang.Primitive // Both are primitives - check if their intersection is empty.
...
This is the inverse of subtyping. See:
...
https://en.wikipedia.org/wiki/Type_system#Type_equivalence
