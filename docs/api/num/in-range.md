...
```typescript
<Min extends number, Max extends number>(value: unknown, min: Min, max: Max) => value is InRange<Min, Max>
```
...
```typescript
;(<Min extends number, Max extends number>(value: number, min: Min, max: Max) =>
  InRange<Min, Max>)
```
...
```typescript
;(<_T extends number, Min extends number, Max extends number>(
  value: _T,
  min: Min,
  max: Max,
) => Clamp<_T, Min, Max>)
