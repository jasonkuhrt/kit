...
When importing from other `@kitz/*` packages, ALWAYS use the namespace export (root path), never the barrel (`/__`):
...
Access members via the namespace (e.g., `Git.Git`, `Fs.Path`).

See `creating-modules` skill for full import system documentation.
